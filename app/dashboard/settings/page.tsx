...
export default function SettingsPage() {
  ...
  const [revealWalletData, setRevealWalletData] = useState<WalletData | null>(
    null,
  );
    ...
    try {
      ...
      if (revealType === "mnemonic") {
        if (!walletData.mnemonic)
          throw new Error("No recovery phrase available for this wallet type.");
  ...
  const handleEnableBiometric = async () => {
    ...
    if (success) {
      ...
      setError(
        "Biometric setup failed. Make sure your device supports it and try again.",
      );
    }
    setLoading(false);
  };
  ...
  const handleNetworkChange = async (
    ...
    if (network === "evm") {
      ...
      setRevealData(
        ECPair.fromPrivateKey(Buffer.from(child.privateKey!), {
  ...
  return (
    ...
    >
      ...
      <Box sx={{ p: 2, flex: 1 }}>
        <Typography
          variant="overline"
          sx={{ px: 1, color: "text.muted", fontWeight: 700 }}
        >
          Chung
        </Typography>
      ...
      >
        ...
        <DialogContent sx={{ py: 2 }}>
          <Alert severity="warning" sx={{ mb: 3, borderRadius: 1.5 }}>
            ...
            Never share this with anyone. Anyone with this info can steal your
            assets.
          </Alert>
          ...
          >
            {!showSensitive ? (
              <>
                <VisibilityOff
                  sx={{ fontSize: 40, color: "text.muted", opacity: 0.5 }}
                />
                <Button
                  variant="outlined"
                  size="small"
          ...
          {success ? (
            ...
            <>
              <Typography variant="body2" color="text.secondary">
                Để kích hoạt bảo mật hệ điều hành, vui lòng xác nhận mật khẩu ví
                của bạn.
              </Typography>
              <TextField
                fullWidth
        ...
        <DialogActions sx={{ p: 3, pt: 0 }}>
          ...
          {!success && (
            <Button
              ...
              disabled={loading || !password}
              startIcon={
                loading ? <CircularProgress size={20} /> : <Fingerprint />
              }
              sx={{ borderRadius: 1.5, px: 3 }}
            >
              Thiết lập
